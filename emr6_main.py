...
def _write_cache(ticker: str, holdings: List[str]) -> None:
    path = os.path.join(CACHE_DIR, f"{ticker}.json")
    payload = {"saved_at": datetime.now().isoformat(), "holdings": holdings}
...
def main() -> None:
    ...
    if not candidate_sources:
        save_current_portfolio(as_of_date, [])
        send_telegram_message(f"[EMR-6 주간 리밸런싱]\n기준일: {as_of_date}\nETF holdings에서 종목 후보를 가져오지 못했습니다.")
        print("ETF holdings에서 종목 후보를 가져오지 못했습니다.")
        return
