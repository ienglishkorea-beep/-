...
def build_report(
    ...
    for i, s in enumerate(final_positions, start=1):
        lines.append(
            ...
            f"비중 {fmt_pct(s.target_weight)} | 1M {fmt_pct(s.ret_1m)} | 3M {fmt_pct(s.ret_3m)} | "
            f"RS {fmt_pct(s.rs_3m_vs_spy)} | ACC {fmt_pct(s.acceleration)} | ETF {s.source_etfs}"
        )
    ...
    if kept_items:
        for x in kept_items:
            lines.append(
                f"- {x['ticker']} {x['name']} | rank {x['rank']} | score {x['score']} | 비중 {fmt_pct(x['target_weight'])}"
            )
    else:
        lines.append("- 없음")
    ...
    if out_items:
        for x in out_items:
            lines.append(
                f"- {x['ticker']} {x['name']} | rank {x['rank']} | score {x['score']}"
            )
    else:
        lines.append("- 없음")
