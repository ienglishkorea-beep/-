...
def build_candidate_sources_from_csv(
    ...
    if subset.empty:
        return candidate_sources, name_map
...
def build_report(
    ...
    lines.append(f"- CSV 후보 원천 {candidate_sources_count}개")
    if final_positions:
    ...
    if final_positions:
        for i, s in enumerate(final_positions, start=1):
            status = "KEEP" if s.ticker in {x.get('ticker') for x in kept_items} else "IN"
            ...
            lines.append(
                f"   - 강점: {summarize_strengths(s)}"
            )
            lines.append(
                f"   - 주의: {summarize_risks(s)}"
            )
            if status == "KEEP":
                lines.append(
                    f"   - 이유: {build_reason_for_keep(s)}"
                )
                lines.append("   - 실행: 기존 보유 유지")
            else:
                lines.append(
                    f"   - 편입 이유: {build_reason_for_in(s)}"
                )
    ...
    if top_etfs_final:
        for i, e in enumerate(top_etfs_final[:8], start=1):
            lines.append(
                f"{i}. {e.ticker} | {e.name} | score {e.score:.3f} | 1M {fmt_pct(e.ret_1m)} | 3M {fmt_pct(e.ret_3m)}"
            )
    else:
        lines.append("- 없음")
...
def build_candidate_lines(stock_scores: List[StockScore]) -> List[str]:
    ...
    for s in stock_scores:
        lines.append(
            ...
            f"M {s.m_score:.3f} | R {s.r_score:.3f} | A {s.a_score:.3f} | V {s.v_score:.3f} | "
            f"1M {fmt_pct(s.ret_1m)} | 3M {fmt_pct(s.ret_3m)} | RS {fmt_pct(s.rs_3m_vs_spy)} | "
            f"ACC {fmt_pct(s.acceleration)} | VOL {fmt_pct(s.vol_20d)} | ETF {s.source_etfs}"
        )
    return lines
...
def main() -> None:
    ...
    candidate_sources, name_map = build_candidate_sources_from_csv(top_etfs_final, holdings_df)
